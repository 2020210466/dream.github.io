...
function Game(id,params){
    ...
    Stage.prototype.createItem = function(options){
        ...
        //关系绑定
        item._stage = this;//添加了一个布景
        item._id = this.items.length;
        this.items.push(item);//新增一个项目
        return item;
    };
    //重置物体位置
    ...
    //绑定事件
    Stage.prototype.bind = function(eventType,callback){//事件监听
        if(!_events[eventType]){
            _events[eventType] = {};
    ...
    this.createStage = function(options){
        var stage = new Stage(options);//实例化了Stage
        stage.index = _stages.length;
        _stages.push(stage);
        return stage;//返回了stage对象
    };
    //指定布景
